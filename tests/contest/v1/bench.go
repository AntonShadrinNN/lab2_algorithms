...
func Bench() ([]int64, []int64, []int64) {
	...
	testRects := make([]rectangles, 11)
	testPoints := make([]points, 11)
	...
	prepTime := make([]int64, 0, 12)
	for i := range testRects {
		fmt.Printf("Testing 2^%d\n", i+1)
		start := time.Now()
		time.Sleep(10 * time.Nanosecond)
		_, _, _ = testRects[i].buildMap()
		prepTime = append(prepTime, time.Since(start).Nanoseconds())
	}

	executionTime := make([]int64, 0, 12)
	for j := range testRects {
		fmt.Printf("Testing 2^%d\n", j+1)

		compX, compY, _ := testRects[j].buildMap()
		start := time.Now()
		time.Sleep(10 * time.Nanosecond)
		for _, v := range testPoints[j] {
			_ = binarySearch(compX, v.x)
			_ = binarySearch(compY, v.y)
		}
		executionTime = append(executionTime, time.Since(start).Nanoseconds())
	}

	totalTime := make([]int64, 0, 12)
	for i := range testRects {
		fmt.Printf("Testing 2^%d\n", i+1)
		start := time.Now()
		time.Sleep(10 * time.Nanosecond)
